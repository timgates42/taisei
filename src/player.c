...
void player_draw_overlay(Player *plr) {
	...
	float char_opacity = min(0.9, min(1, a * 5) * char_out * char_out);
	...
	for(int i = 1; i <= 3; ++i) {
		...
		r_draw_sprite(&(SpriteParams) {
			...
			.color = color_mul_scalar(color_add(RGBA(0.2, 0.2, 0.2, 0), RGBA(i==1, i==2, i==3, 0)), char_opacity * (1 - char_in * o) * o),
			.flip.x = true,
